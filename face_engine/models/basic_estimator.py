...
class BasicEstimator(Estimator, name="basic"):
    ...
    def predict(self, embeddings):
        ...
        # (M, N) = (M, 1) + (N,) - 2 * (M, D) @ (D, N)
        dists_sq = input_norms_sq + self.fitted_norms_sq - 2 * embeddings @ self.embeddings.T
    ...
    def load(self, dirname):
        ...
        # Ensure fitted_norms_sq is available for models saved with older versions
        if getattr(self, "fitted_norms_sq", None) is None and self.embeddings is not None:
            self.fitted_norms_sq = np.sum(self.embeddings**2, axis=1)
