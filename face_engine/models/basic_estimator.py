...
class BasicEstimator(Estimator, name="basic"):
    ...
    def predict(self, embeddings):
        ...
        # Handle potential small negative values due to floating point errors
        dists_sq = np.maximum(dists_sq, 0)

        indices = np.argmin(dists_sq, axis=1)
        scores = np.exp(-0.5 * dists_sq[np.arange(len(embeddings)), indices])
    ...
    def load(self, dirname):
        ...
        # Backward compatibility: ensure fitted_norms_sq is present
