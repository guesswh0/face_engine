...
class BasicEstimator(Estimator, name="basic"):
    ...
    def predict(self, embeddings):
        ...
        dists_sq = (
            ...
            + query_norms_sq[:, np.newaxis]
            - 2 * np.dot(embeddings, self.embeddings.T)
        )
        # Ensure distances are non-negative due to floating point precision issues
        dists_sq = np.maximum(dists_sq, 0.0)

        indices = np.argmin(dists_sq, axis=1)
        best_dists_sq = dists_sq[np.arange(len(embeddings)), indices]

        scores = np.exp(-0.5 * best_dists_sq).tolist()
    ...
    def load(self, dirname):
        ...
        # Re-calculate fitted_norms_sq after loading for backward compatibility
